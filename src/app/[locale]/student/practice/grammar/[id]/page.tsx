...
import grammarPracticeService from '@/lib/services/grammar-practice.service'; // Import service mới
import { Lesson } from '@/lib/types/lesson'; // Import Lesson type
...
interface LessonData {
    title: string;
    description: string;
    exercises: any[]; // Grammar practice có field exercises
    id: number;
}
        ...
        const initializeData = async () => {
            ...
            if (stored) {
                try {
                    ...
                    // Đảm bảo đây là dữ liệu grammar practice và có exercises
                    if (dataFromStorage && dataFromStorage.exercises && dataFromStorage.exercises.length > 0) {
                        setLessonData(dataFromStorage);
                        setLoading(false);
                        return; // Không cần fetch API nếu có dữ liệu hợp lệ trong localStorage
                    }
                } catch (e) {
            ...
            // Nếu không có dữ liệu hợp lệ trong localStorage hoặc exercises rỗng, fetch từ API
